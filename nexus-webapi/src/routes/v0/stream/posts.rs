...
#[utoipa::path(
    ...
    params(
        ...
        ("tags" = Option<Vec<String>>, Query, description = "Filter by a list of comma-separated tags (max 5). E.g.,`&tags=dev,free,opensource`. Results are ordered by the number of matching tags in descending order (posts with all N tags first, then N-1 tags, then N-2 tags, etc.)."),
