...
impl NexusApi {
    ...
    async fn start_icann_http_server(
        ...
        let server = axum_server::from_tcp(listener).inspect_err(|e| {
    ...
    async fn start_pubky_tls_server(
        ...
        let server = axum_server::from_tcp(pubky_listener).inspect_err(|e| {
            error!("Failed to create Pubky server from TCP listener: {e}");
        })?;
