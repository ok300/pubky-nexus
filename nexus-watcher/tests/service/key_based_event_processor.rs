...
use nexus_watcher::service::{KeyBasedEventProcessor, KeyBasedRawEvent, RunError, TEventProcessor};
use pubky::Keypair;
...
async fn key_based_processor_skips_parse_errors_and_continues_user_events() -> Result<(), DynError>
{
    ...
    let source = Arc::new(MockKeyBasedEventSource::default().with_user_events(
        &user_id,
        vec![
            KeyBasedRawEvent {
                cursor_id: 1,
                event_line: "NOT_AN_EVENT".into(),
            },
            KeyBasedRawEvent {
                cursor_id: 2,
                event_line: format!("PUT pubky://{user_id}/pub/pubky.app/profile.json"),
            },
        ],
    ));
...
async fn key_based_processor_continues_to_next_user_after_parse_error_then_rejects_wrong_user_event(
    ...
    let source = Arc::new(MockKeyBasedEventSource::default().with_call_events(vec![
        vec![KeyBasedRawEvent {
            cursor_id: 1,
            event_line: "NOT_AN_EVENT".into(),
        }],
        vec![
            KeyBasedRawEvent {
                cursor_id: 2,
                event_line: format!("PUT pubky://{different_user_id}/pub/pubky.app/profile.json"),
            },
            KeyBasedRawEvent {
                cursor_id: 3,
                event_line: format!(
                    "PUT pubky://{different_user_id}/pub/pubky.app/posts/after-mismatch"
                ),
            },
        ],
    ]));
    ...
    assert!(matches!(
        result,
        Err(RunError::Internal(EventProcessorError::UserIdMismatch(_)))
    ));
