...
async fn test_delete_pubkyapp_file() -> Result<()> {
    ...
    // Act
    let files_before_delete = FileDetails::get_by_ids(&[&[user_id.as_str(), file_id.as_str()] as &[&str]])
        .await
    ...
    // Assert
    let files = FileDetails::get_by_ids(&[&[user_id.as_str(), file_id.as_str()] as &[&str]])
        .await
        .expect("Failed to fetch files from Nexus");
