...
async fn test_get_all_active_excludes_orphan_homeservers() -> Result<()> {
    ...
    let link_query = queries::put::set_user_homeserver(&user_id, &test.homeserver_id);
    ...
    assert!(
        active_ids.contains(&test.homeserver_id),
