...
impl Homeserver {
    ...
    /// Retrieves from the graph the IDs of homeservers that have at least one
    /// active user, excluding the specified default homeserver.
    ///
    /// The returned list is sorted by the number of active users in descending
    /// order.
    ///
    /// # Arguments
    /// * `default_hs_id` – The homeserver ID to exclude from the results
    ///   (typically the default/primary homeserver, which is processed separately).
    ///
    /// # Returns
    /// A list of homeserver IDs.
    ///
    /// # Errors
    /// Returns an error if no matching homeservers are found.
    pub async fn get_all_from_graph(default_hs_id: &str) -> GraphResult<Vec<String>> {
        let query = queries::get::get_active_homeservers(default_hs_id);
        let maybe_hs_ids = fetch_key_from_graph(query, "homeserver_ids").await?;
